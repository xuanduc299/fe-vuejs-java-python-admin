...
const chooseProductForOrderObj = reactive<ChooseProductForORderModalType>({
  visible: false,
  onOk: (product: any) => {
    console.log("on ok choose product: ", product);
    if (
    ...
    .then((res) => {
      ...
      res.data.chiTietDonHang.forEach((item) => {
        if (pMap.get("id_" + item.sanPham.id)) {
          ...
          pMap.set("id_" + item.sanPham.id, [
            {
              label: item.motaPhanLoai,
              quantity: item.soLuong,
              variationId: item.phanLoaiSpId,
              isShowErr: false,
              maxQuantity: 999,
              product: item.sanPham,
            },
      ...
      choseProductList.value = newArr;

      const orderRes = res.data;
      orderInfo.ghiChu = orderRes.ghiChu;
      orderInfo.discount = orderRes.tongTienGiamGia;
      orderInfo.shipFee = orderRes.phiShip;
      orderInfo.phuongThucTT = orderRes.phuongThucTT;
      try {
        const orderAddressRes = orderRes?.diaChi?.diaChi.split("__");
        orderInfo.diaChiNhanHang = orderAddressRes[0];
        addressInfo.province = orderAddressRes[3];
        onProvinceChange();
