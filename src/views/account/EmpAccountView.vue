<template>
  <div id="emp_account">
    <a-page-header class="rounded-[5px]"
      style="border: 1px solid rgb(235, 237, 240); margin: 20px; background-color: white; height: 100%"
      ...
      <a-table class="mt-[20px]" :columns="tblConfig.columns" :data-source="tblConfig.data" :loading="tblConfig.loading"
        ...
        <template #bodyCell="{ column, record, text }">
          ...
          <template v-else-if="column.key === 'role'">
            <template v-if="text === 'ROLE_ADMIN'"> ADMIN </template>
            <template v-if="text === 'ROLE_STAFF'"> Nhân viên </template>
            <template v-if="text === 'ROLE_USER'"> Khách hàng </template>
          </template>
    ...
    addEditUserModalInstance.formRef
      ...
      .then((values) => {
        ...
        if (addEditUserModalInstance.type === "add")
          UserService.addUser({ ...values, role: "ROLE_STAFF" })
            .then((res) => {
